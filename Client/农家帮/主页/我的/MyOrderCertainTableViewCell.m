...
@implementation MyOrderCertainTableViewCell

{
    ...
    UIImageView * _hasQueRenImageView;//已确认的图片背景
}
- (void)awakeFromNib {
    // Initialization code
}
...
- (void)createUI2
{
    ...
    _hasQueRenImageView = [[UIImageView alloc]init];
    ...
    [_backView addSubview:_hasQueRenImageView];
    [self DTsetyueshu];
}
...
- (void)DTsetyueshu
{
    ...
    _imageView.image = [UIImage imageNamed:@"grzx_wddd_radio_sel.png"];
    [_querenlable mas_remakeConstraints:^(MASConstraintMaker *make) {
        CGSize size = [@"已确认" sizeWithFont:BaseFont(18) maxSize:CGSizeMake(999, 24)];
        make.right.mas_equalTo(-40 * scaleWidth);
        make.width.mas_equalTo(size.width);
        make.centerY.mas_equalTo(_certainOrder.mas_centerY);
        make.height.mas_equalTo(80 * scaleHeight);
    }];
    _querenlable.text = @"已确认";
    ...
    [_hasQueRenImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(147 * scaleWidth);
        make.height.mas_equalTo(183 * scaleHeight);
        make.centerY.mas_equalTo(_backView.mas_centerY);
        make.right.mas_equalTo(0);
    }];
    _hasQueRenImageView.image = [UIImage imageNamed:@"grzx_wddd_YQR.png"];
...
- (void)setModel:(myOrderModel *)model
{
    ...
    NSDictionary* style1 = @{@"body":[UIFont systemFontOfSize:16],
