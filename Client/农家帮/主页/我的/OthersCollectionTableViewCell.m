//
//  OthersCollectionTableViewCell.m
//  农家帮
//
...
@implementation OthersCollectionTableViewCell
{
    UIImageView * _headerImage;
    UIImageView *_icon;
    UILabel *_name;
    UIImageView *_renzhengImageView;
    
    UILabel * _GoodsNameLable;
    UIButton * _cateGory;//商品的类型
    UILabel * _goodsMessageLable;//商品的信息
    UIView * _backView;
    UIButton * _siliaoButton;//私聊按钮
...
- (void)createUI
{
    ...
    _siliaoButton = [[UIButton alloc]init];
...
- (void)DTsetyueshu
{
    ...
    [_siliaoButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(90 * scaleHeight);
        make.width.mas_equalTo(119 * scaleWidth);
        make.right.mas_equalTo(-20 * scaleWidth);
        make.height.mas_equalTo(39 * scaleHeight);
    }];
    [_siliaoButton setBackgroundImage:[UIImage imageNamed:@"cpxq_btn_siliao.png"] forState:UIControlStateNormal];
    [_siliaoButton addTarget:self action:@selector(siliaoButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    
    
}

- (void)siliaoButtonClick:(UIButton *)button
{
    _Myblock(button);
}
