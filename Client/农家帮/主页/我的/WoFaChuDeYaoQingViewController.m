//
//  WoFaChuDeYaoQingViewController.m
//  农家帮
//
...
@interface WoFaChuDeYaoQingViewController ()<UITableViewDataSource, UITableViewDelegate, MJRefreshBaseViewDelegate>

@end

@implementation WoFaChuDeYaoQingViewController
{
...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"我发出的邀请", @"");
...
- (void)requestDataWithOffSet:(NSString *)OffSet DataLimit:(NSString *)limit {
    ...
    [[JudgeNetState shareInstance]monitorNetworkType:self.view complete:^(NSInteger state) {
        if (state == 1 || state == 2) {
            ...
            [_JQResquest piPeiYaoQingXiaoXiLieBiaoWithProducerId:_userId complete:^(NSDictionary *responseObject) {
...
//    cell.imageView.layer.cornerRadius = 10;
    ...
    WoFaChuDeYaoQingTableViewCell *yaoqingCell = [tableView dequeueReusableCellWithIdentifier:@"yaoqingCell"];
    if (!yaoqingCell) {
        yaoqingCell = [[WoFaChuDeYaoQingTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"yaoqingCell"];
        
    }
    if (_dataArray.count) {
        NSDictionary *dict = _dataArray[indexPath.row];
        [yaoqingCell.picture sd_setImageWithURL:dict[@"smallportraiturl"] placeholderImage:[UIImage imageNamed:@"grzx_icon_01"]];
        yaoqingCell.name.text = [NSString stringWithFormat:@"%@",dict[@"displayname"]];
        
        UILabel *line = [[UILabel alloc] initWithFrame:CGRectMake(0, 63, Screen_Width, 1)];
        line.backgroundColor = BaseColor(233, 233, 233, 1);
        [yaoqingCell addSubview:line];
    }
    return yaoqingCell;
