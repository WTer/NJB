//
//  XiaoFeiZheYouXiaoTableViewCell.m
//  农家帮
//
...
@implementation XiaoFeiZheYouXiaoTableViewCell
...
- (void)createUI
{
    ...
    _suoyouquanLable = [[UILabel alloc]init];
    [_imageView addSubview:_suoyouquanLable];
...
- (void)reSetKongjianWeizhi
{
    ...
    [_suoyouquanLable mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(_imageView.mas_right).offset(-53 * scaleWidth);
        make.bottom.mas_equalTo(_imageView.mas_bottom).offset(-16 * scaleHeight);
        make.height.mas_equalTo(30 * scaleHeight);
        make.width.mas_equalTo(200 * scaleWidth);
    }];
    ...
    [_sanchuButton setTitleColor:BaseColor(225, 225, 225, 1) forState:UIControlStateNormal];
    
    _xiugaiButton.titleLabel.font = BaseFont(30);
    [_xiugaiButton setTitle:@"修改" forState:UIControlStateNormal];
    [_xiugaiButton setTitleColor:BaseColor(254, 72, 48, 1) forState:UIControlStateNormal];
    [_xiugaiButton setBackgroundImage:[UIImage imageNamed:@"zjhh_btn_xiugai_up.png"] forState:UIControlStateNormal];
    _imageView.image = [UIImage imageNamed:@"zjhb_ico_youxiaozhong.jpg"];
...
- (void)DingyiCicun
{
    ...
    [_nameNumberLable mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(328 * scaleWidth);
        make.top.mas_equalTo(30 * scaleHeight);
        make.height.mas_equalTo(40 * scaleHeight);
        make.left.mas_equalTo(120 * scaleWidth);
    ...
    [_senderButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(160 * scaleHeight);
        make.left.mas_equalTo(209 * scaleWidth);
        make.height.mas_equalTo(56 * scaleHeight);
        make.width.mas_equalTo(177 * scaleWidth);
        
    }];
    [_suoyouquanLable mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(_senderButton.mas_left);
        make.top.mas_equalTo(_senderButton.mas_bottom).offset(10 * scaleHeight);
        make.height.mas_equalTo(30 * scaleHeight);
        make.width.mas_equalTo(200 * scaleWidth);
    }];
    [_imageView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.width.mas_equalTo(460 * scaleWidth);
        make.top.mas_equalTo(11 * scaleHeight);
        make.height.mas_equalTo(261 * scaleHeight);
        make.left.mas_equalTo(140 * scaleWidth);
    }];
    [_zhuangtaiImageView mas_remakeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(0);
        make.right.mas_equalTo(0);
    ...
    [_xiugaiButton mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(0 * scaleHeight);
        make.left.mas_equalTo(_imageView.mas_right).offset(0);
        make.top.mas_equalTo(_sanchuButton.mas_top).offset(7 * scaleHeight);
    }];
    _imageView.image = [UIImage imageNamed:@"zjhb_ico_(close)youxiaozhong.jpg"];
}

/*更具数据加载界面次从**/
- (void)jiazaiCicong
{
    _zhuangtaiImageView.image = [UIImage imageNamed:@"zjhb_ico_(close)youxiaozhong.jpg"];
    
    _suoyouquanLable.text = @"红包所有权归商家所有";
    _suoyouquanLable.textColor= BaseColor(255, 224, 1, 1);
    _suoyouquanLable.adjustsFontSizeToFitWidth = YES;
    ...
    _nameNumberLable.textColor = BaseColor(225, 225, 225, 1);
    _nameNumberLable.textAlignment = NSTextAlignmentCenter;
    _nameNumberLable.adjustsFontSizeToFitWidth = YES;
    
    _timeNumberLable.text = [NSString stringWithFormat:@"有效期：%@", _Time];
    ...
    [_senderButton setTitle:@"未领取" forState:UIControlStateNormal];
    [_senderButton setBackgroundColor:BaseColor(254, 72, 48, 1)];
    _senderButton.layer.cornerRadius =6;
    [_senderButton setTitleColor:BaseColor(254, 72, 48, 1) forState:UIControlStateNormal];
    _senderButton.backgroundColor = BaseColor(255, 224, 1, 1);
