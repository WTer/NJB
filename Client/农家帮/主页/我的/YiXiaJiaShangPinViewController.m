//
//  YiXiaJiaShangPinViewController.m
//  农家帮
//
...
@interface YiXiaJiaShangPinViewController ()<UITableViewDataSource, UITableViewDelegate, MJRefreshBaseViewDelegate>

@end

@implementation YiXiaJiaShangPinViewController
{
...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"已下架的商品", @"");
...
- (void)requestDataWithOffSet:(NSString *)OffSet DataLimit:(NSString *)limit {
    ...
    [[JudgeNetState shareInstance]monitorNetworkType:self.view complete:^(NSInteger state) {
        if (state == 1 || state == 2) {
            ...
            [_JQResquest GetProducerXiaJiaShangPinListWithProducerId:_userId complete:^(NSDictionary *responseObject) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_dataArray.count) {
        ...
        CGSize sizePrice = [price sizeWithFont:[UIFont systemFontOfSize:27.0] maxSize:CGSizeMake(999, 100)];
        CGSize sizeName = [name sizeWithFont:[UIFont systemFontOfSize:21.0] maxSize:CGSizeMake(999, 100)];
        
        
        CGSize size = [description sizeWithFont:[UIFont systemFontOfSize:18.0] maxSize:CGSizeMake(999, 1000)];
        ...
        return size.height * detailCount + sizePrice.height + sizeName.height + 220 * scaleHeight;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_dataArray.count) {
        NSDictionary *dict = _dataArray[indexPath.row];
        [cell configWithDictionary:dict withAll:NO];
    }
    return cell;
