//
//  ZengJiaYouHuiQuanViewController.m
//  农家帮
//
...
@interface ZengJiaYouHuiQuanViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,UIScrollViewDelegate>

@end

@implementation ZengJiaYouHuiQuanViewController
{
...
- (void)viewDidLoad {
    ...
    self.title = NSLocalizedString(@"添加优惠券", @"");
...
- (void)createUI {
    ...
    _youhuiquanTF.placeholder = @"优惠券名称";
    ...
    _jiageTF.placeholder = @"请输入优惠券优惠金额";
...
//    _jiageTF.rightViewMode = UITextFieldViewModeAlways;
    ...
    _tiaojianTF = [[UITextField alloc] initWithFrame:CGRectMake(24 * scaleWidth, 312 * scaleHeight, 672 * scaleWidth, 75 * scaleHeight)];
    _tiaojianTF.backgroundColor = BaseColor(249, 249, 249, 249);
    _tiaojianTF.layer.cornerRadius = 10 * scaleHeight;
    _tiaojianTF.placeholder = @"请输入可使用优惠券的价格条件";
    _tiaojianTF.delegate = self;
    _tiaojianTF.font = [UIFont systemFontOfSize:14.0];
    [_mainScrollView addSubview:_tiaojianTF];
    UILabel *tiaojian = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 120 * scaleWidth, 75 * scaleHeight)];
    tiaojian.font = [UIFont systemFontOfSize:16.0];
    tiaojian.text = @"条件";
    tiaojian.textAlignment = NSTextAlignmentCenter;
    tiaojian.textColor = BaseColor(105, 105, 105, 1);
    _tiaojianTF.leftView = tiaojian;
    _tiaojianTF.leftViewMode = UITextFieldViewModeAlways;
    ...
    [[JudgeNetState shareInstance] monitorNetworkType:self.view complete:^(NSInteger state) {
        ...
        if (state == 1 || state == 2) {
            if (_youxiaoqiTF.text.length == 0 || _jiageTF.text.length == 0 || _youxiaoqiTF.text.length == 0 || _jiezhiriqiTF.text.length == 0) {
                ...
                [_jqRequest XinZengYouHuiQuanPOSTWithProducerId:[NSString stringWithFormat:@"%@",[ud objectForKey:@"ProducerId"]] Name:_youhuiquanTF.text Amount:_jiageTF.text MiniCharge:_tiaojianTF.text StartTime:_youxiaoqiTF.text ExpirationTime:_jiezhiriqiTF.text complete:^(NSDictionary *responseObject) {
                    
                    UIAlertController*alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"ATTENTION", @"") message:[NSString stringWithFormat:@"%@商品优惠券增加成功",_youhuiquanTF.text] preferredStyle:UIAlertControllerStyleAlert];
                    UIAlertAction*otherAction = [UIAlertAction  actionWithTitle:NSLocalizedString(@"OK", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction*action) {
...
//                [_jqRequest HuoQuProducerYiFaBuShangPinListWithProducerId:[NSString stringWithFormat:@"%@",[ud objectForKey:@"ProducerId"]] complete:^(NSDictionary *responseObject) {
//
//
...
//                        if ([_youhuiquanTF.text isEqualToString:[NSString stringWithFormat:@"%@",dict[@"name"]]] == YES) {
//                            
//                            
//                            
//                        }
...
//                        if (num == array.count) {
//                            
//                            UIAlertController*alertController = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"ATTENTION", @"") message:@"优惠券名称不是您所发布的商品名称" preferredStyle:UIAlertControllerStyleAlert];
//                            UIAlertAction*otherAction = [UIAlertAction  actionWithTitle:NSLocalizedString(@"OK", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction*action) {
//                                
//                                
//                                
